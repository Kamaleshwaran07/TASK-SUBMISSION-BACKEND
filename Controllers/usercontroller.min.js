...
const today = format(new Date(), 'dd-MM-yyyy, h:mm a')
dotenv.config();
...
const userContoller = {
    ...
    posttaskurls: async (req, res) => {
        try {
            ...
            const taskupdate = await Task.findOneAndUpdate({taskId: utaskId}, { frontEndCode: frontendcode, frontEndUrl: frontendurl, backEndUrl: backendurl, backEndCode: backendcode, user: userId, status: "Submitted", comment: null, score: null, updatedAt: today })
    ...
    getTaskUrls: async (req, res) => { 
        ...
        try {
            ...
            //Conditional rendering of urls
                ...
                return res.status(200).json({message:"Successfully fetched submitted tasks", submittedTask, pendingTask })
            }
                ...
                const pendingTask = await Task.find({ status: "Pending" }).select('-_id -status -__v ')
    ...
    createTask: async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({message:"Task created successfully", taskSave})
