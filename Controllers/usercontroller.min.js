...
const userContoller = {
    ...
    posttaskurls: async (req, res) => {
        try {
            ...
            const taskupdate = await Task.findOneAndUpdate({taskId: utaskId}, { frontEndCode: frontendcode, frontEndUrl: frontendurl, backEndUrl: backendurl, backEndCode: backendcode, user: userId, status: "Submitted", comment:"Not yet Graded", score:"Not yet Graded", updatedDate: todaydate, updateTime: todaytime })
    ...
    getTaskUrls: async (req, res) => { 
        ...
        try {
            ...
            const comments = await Task.find({ status: { $ne: "Pending" } }).select('-_id -__v ')
                // console.log("comment", comments);
                const pendingTask = await Task.find({ status: "Pending" }).select('-_id -__v ')
                
                    
                    res.status(200).json({comments,pendingTask, message:"Successfully fetched the score"})
